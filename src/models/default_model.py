"""Default Prediction Modeling (Baseline & Improved Calibrated LightGBM)."""
...
class DefaultModel:
    """Predicts next 12-month loan default / charge-off probability."""
    ...
    def train_baseline(self, X_train: pd.DataFrame, y_train: pd.Series) -> LogisticRegression:
        """Trains baseline regularized Logistic Regression with balanced weighting."""
        logger.info(f"Training Baseline Default Model (LogisticRegression) on {len(X_train):,} samples...")
    ...
    def train_improved(
        ...
        logger.info(f"Training Improved Default Model (LightGBM) on {len(X_train):,} samples...")
        ...
        if self.use_calibration:
            logger.info("Applying Isotonic Probability Calibration to Default Model...")
    ...
    def predict_proba(self, X: pd.DataFrame, model_type: str = "improved") -> np.ndarray:
        """Predicts calibrated default probabilities."""
    ...
    def evaluate(
        ...
        if self.baseline_model is not None:
            ...
            baseline_metrics = evaluate_binary_classifier(
                y_test.to_numpy(), base_prob, threshold=threshold, model_name="Baseline_Default_LogReg"
            )
        ...
        if self.improved_model is not None:
            ...
            improved_metrics = evaluate_binary_classifier(
                y_test.to_numpy(), imp_prob, threshold=threshold, model_name="Improved_Default_LightGBM"
            )
