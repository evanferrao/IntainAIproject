"""Delinquency Prediction Modeling (Baseline & Improved Calibrated LightGBM)."""
...
class DelinquencyModel:
    """Predicts next 3-month delinquency flag (30+ DPD incidence)."""
    ...
    def train_baseline(self, X_train: pd.DataFrame, y_train: pd.Series) -> LogisticRegression:
        """Trains baseline regularized Logistic Regression with balanced class weighting."""
        logger.info(f"Training Baseline Delinquency Model (LogisticRegression) on {len(X_train):,} samples...")
    ...
    def train_improved(
        ...
        logger.info(f"Training Improved Delinquency Model (LightGBM) on {len(X_train):,} samples...")
        ...
        if self.use_calibration:
            logger.info("Applying Isotonic Probability Calibration via 3-Fold Cross-Validation...")
    ...
    def predict_proba(self, X: pd.DataFrame, model_type: str = "improved") -> np.ndarray:
        """Predicts calibrated positive class probabilities."""
    ...
    def evaluate(
        ...
        if self.baseline_model is not None:
            ...
            baseline_metrics = evaluate_binary_classifier(
                y_test.to_numpy(), base_prob, threshold=threshold, model_name="Baseline_Delinquency_LogReg"
            )
        ...
        if self.improved_model is not None:
            ...
            improved_metrics = evaluate_binary_classifier(
                y_test.to_numpy(), imp_prob, threshold=threshold, model_name="Improved_Delinquency_LightGBM"
            )
